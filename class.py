# Create student class and possible function and attributes. 
# Using constructor func,set and get function
# class student:
#     def __init__(self,name,subject,rank):
#         self.name=name
#         self.subject=subject
#         self.rank=rank
#     def get_function(self):
#         print("name:",self.name) 
#         print("subject:",self.subject)
#         print("rank:",self.rank)
#     def set_subject_function(self,subject):
#         self.subject=subject
#     def set_name_function(self,name):
#         self.name=name
#     def set_rank_function(self,rank):
#         self.rank=rank 
#         student1.get_function()
# student1= student("g1","maths",1)
# student2= student("g2","english",2)
# student3= student("g3","computer",3) 
# student1.get_function()

# student1.subject="social"####change subject
# student1.get_function()

# student2.get_function()
# student3.get_function()





# Create a class called laptop.Create a following variables and function.
# variable=price,proccssor,ram Create object as Lenovo, HP,Dell
# class Laptop:
#     def __init__(self,price,processor,ram):
#        self.price=price
#        self.processor=processor
#        self.ram=ram
#     def get_function(self):
#        print("price:",self.price)
#        print("processor:",self.processor)
#        print("ram:",self.ram)
#     def set_price_function(self,price):
#         self.price=price
#     def set_processor_function(self,processor):
#         self.processor=processor
#     def set_ram_function(self,ram):
#         self.ram=ram        
# Lenovo=Laptop(500,"intel i7",16)
# HP=Laptop(600,"amd",8)
# Dell=Laptop(900,"intel",10)  
# Lenovo.get_function()

# Lenovo.price=2000#####change price
# Lenovo.get_function()
# HP.get_function()
# Dell.get_function()



#Create class kodaikanal and create all the possible variable and function
# class Kodaikanal:
#     def __init__(self,name,place,age,modeoftravel):
#         self.name=name
#         self.place=place
#         self.age=age
#         self.modeoftravel=modeoftravel
#     def get_function(self):
#         print("name:",self.name) 
#         print("place:",self.place)
#         print("age:",self.age) 
#         print("modeoftravel:",self.modeoftravel) 
#     def set_name_function(self,name): 
#         self.name=name
#     def set_place_function(self,place):
#         self.place=place
#     def set_age_function(self,age):
#         self.age=age
#     def set_modeoftravel_function(self,modeoftravel):
#         self.modeoftravel=modeoftravel
# MountainVisit=Kodaikanal("Tourist1","Chennai",25,"Train")
# Falls=Kodaikanal("Tourist2","Coimbatore",23,"Flight") 
# Foodexplore=Kodaikanal("Tourist3","Madurai",26,"Bus")  
# MountainVisit.get_function()
# Falls.get_function() 
# Foodexplore.get_function()








